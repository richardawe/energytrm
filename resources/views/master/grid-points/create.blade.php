<x-app-layout><x-slot name="title">Add Grid Point — {{ $index->index_name }}</x-slot>
<div class="mb-3">
    <a href="{{ route('master.indices.show', $index) }}" class="text-muted small text-decoration-none">← {{ $index->index_name }}</a>
</div>
<div class="card card-etrm" style="max-width:580px;"><div class="card-header">Add Grid Point — {{ $index->index_name }}</div><div class="card-body">
<form method="POST" action="{{ route('master.indices.grid-points.store', $index) }}">@csrf
<div class="row g-3">
    <div class="col-md-6">
        <label class="form-label fw-semibold">Price Date <span class="text-danger">*</span></label>
        <input type="date" name="price_date" class="form-control @error('price_date') is-invalid @enderror" value="{{ old('price_date') }}" required>
        @error('price_date')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>
    <div class="col-md-6">
        <label class="form-label fw-semibold">Price <span class="text-danger">*</span></label>
        <input type="number" name="price" step="0.000001" class="form-control @error('price') is-invalid @enderror" value="{{ old('price') }}" required>
        @error('price')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>

    <div class="col-md-8">
        <label class="form-label fw-semibold">Grid Point Label</label>
        <input type="text" name="grid_point_label" class="form-control @error('grid_point_label') is-invalid @enderror" value="{{ old('grid_point_label') }}" maxlength="100">
    ...
    <div class="col-md-4">
        ...
        <input type="number" name="priority_level" class="form-control @error('priority_level') is-invalid @enderror" value="{{ old('priority_level') }}" min="1" max="8">
    ...
    <div class="col-md-6">
        ...
        <select name="instrument_category" class="form-select @error('instrument_category') is-invalid @enderror">
            ...
            <option value="{{ $cat }}" {{ old('instrument_category') == $cat ? 'selected' : '' }}>{{ $cat }}</option>
            @endforeach
        </select>
    ...
    <div class="col-md-6">
        ...
        <select name="sensitivity" class="form-select @error('sensitivity') is-invalid @enderror">
            ...
            <option value="{{ $sens }}" {{ old('sensitivity') == $sens ? 'selected' : '' }}>{{ ucfirst($sens) }}</option>
            @endforeach
        </select>
    ...
    <div class="col-md-6">
        ...
        <input type="number" name="delta_shift" step="0.000001" class="form-control @error('delta_shift') is-invalid @enderror" value="{{ old('delta_shift') }}">
