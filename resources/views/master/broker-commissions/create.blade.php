<x-app-layout>
    <x-slot name="title">Add Commission — {{ $broker->name }}</x-slot>
    ...
    <div class="card card-etrm" style="max-width:640px;">
        <div class="card-header">Add Commission Schedule — {{ $broker->name }}</div>
        <div class="card-body">
            <form method="POST" action="{{ route('master.brokers.commissions.store', $broker) }}">
                @csrf
                <div class="row g-3">
                    <div class="col-12">
                        <label class="form-label fw-semibold">Schedule Name <span class="text-danger">*</span></label>
                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                               value="{{ old('name') }}" maxlength="100" placeholder="e.g. Standard OTC Rate">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="rate_unit" class="form-control"
                               value="{{ old('rate_unit') }}" maxlength="50" placeholder="e.g. USD/lot, bps">
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="payment_frequency" class="form-select @error('payment_frequency') is-invalid @enderror">
                            @foreach(['Per Trade','Monthly','Quarterly'] as $f)
                                <option value="{{ $f }}" {{ old('payment_frequency','Per Trade') == $f ? 'selected' : '' }}>{{ $f }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="index_group" class="form-control"
                               value="{{ old('index_group') }}" maxlength="100" placeholder="e.g. NBP Gas, ICE Brent">
                    </div>
                ...
                <div class="d-flex gap-2 mt-3">
                    <button type="submit" class="btn btn-primary"
                            style="background:var(--etrm-secondary);border-color:var(--etrm-secondary);">Add Commission</button>
